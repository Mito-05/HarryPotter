...
async function fetchPotterDatta(characterName) {
    try{
...
function displayPotterInfo(character){
    const { name , house, species, gener, ancestry, wand,patronus, image } = character;

    potterInfoDiv.innerHTML = `
        ${image ? `img src='${image}' alt='${name}'`:''}
        <h3>${name}</h3>
        <p>
        <strong>Casa:</strong> ${house|| 'Desconocida'}
        </p>
        <p>
